...
public class DrawingTUI {
	...
	public void afficherForm(Forme f) {
		if (f instanceof Cercle) {
			...
			System.out.print("centre=(" + ((Cercle) f).getCentre().get(0).getX() + ","
					+ ((Cercle) f).getCentre().get(0).getY() + "),");
		...
		if (f instanceof Carre) {
			...
			for (int i = 0; i < 4; i++) {
				...
				System.out.print("Point" + "=(" + ((Carre) f).getcoordonnes().get(i).getX() + ","
						+ ((Carre) f).getcoordonnes().get(i).getY() + "),");
			}
		...
		if (f instanceof Rectangle) {
			...
			for (int i = 0; i < 4; i++) {
				...
				System.out.print("Point" + (i + 1) + "=(" + ((Rectangle) f).getcoordonnes().get(i).getX() + ","
						+ ((Rectangle) f).getcoordonnes().get(i).getY() + "),");
			}
		...
		if (f instanceof Triangle) {
			...
			for (int i = 0; i < 3; i++) {
				...
				System.out.print("Point" + (i + 1) + "=(" + ((Triangle) f).getOrigine().get(i).getX() + ","
						+ ((Triangle) f).getOrigine().get(i).getY() + ")");
				if (i != 2) {
					System.out.print(",");
				}
	...
	private Forme ExcuteCommandeQuit(String string, String commande) {
		System.out.println("toutes les formes et les dessins que vous avez créer sont sauvgarder");
		System.exit(0);
		return null;
	}
	...
	private Forme excuteCommandeMove(String NomCommande, String commande) throws Exception {
		...
		if (commande == null) {
			...
			d.close();
			if (d.find(valeur.get(1)) != null) {
				return d.update(commandMove.excute(d.find(valeur.get(1)), Integer.parseInt(valeur.get(2)),
						Integer.parseInt(valeur.get(3))));
			} else {
				if (c.find(valeur.get(1)) != null) {
					return c.update((Dessin) commandMove.excute(c.find(valeur.get(1)), Integer.parseInt(valeur.get(2)),
							Integer.parseInt(valeur.get(3))));
				} else {
				...
				if (cdd.find(valeurs.get(0)) != null) {
					if ((ddf.find(valeurs.get(2)) != null)) {
						if (cdd.find(valeurs.get(0)).nom != ddf.find(valeurs.get(2)).nomGRP) {
							...
							return (Dessin) cdd
									.update(commande.excute(cdd.find(valeurs.get(0)), ddf.find(valeurs.get(2))));
						} else {
						...
						if (cdd.find(valeurs.get(2)) != null) {
							...
							if ((cdd.find(valeurs.get(0)).nom != cdd.find(valeurs.get(2)).nomGRP)) {
								return (Dessin) cdd
										.update(commande.excute(cdd.find(valeurs.get(0)), cdd.find(valeurs.get(2))));
							} else {
								System.out.println("le dessin contient deja la forme que vous voullez inserer");
					...
					System.out.println("la dessin:" + " " + valeurs.get(0) + " n'existe pas");
				}
		...
		if (commande == null) {
			...
			try {
				...
				if (ddf.find(valeurs.get(0)) == null) {
					...
					return ddf.createCercle(
							commande.excute(valeurs.get(0), Point.PointFactory(origineX, origineY), rayon));
				} else {
	...
	public Triangle excuteCommandeTriangle(String NomCommande, String commandeBrute) {
		...
		try {
			...
			if (ddf.find(valeurs.get(0)) == null) {
				...
				if (verifieCoordone(points.get(0), points.get(1)) == false) {
					if (verifieCoordone(points.get(0), points.get(2)) == false) {
						if (verifieCoordone(points.get(2), points.get(1)) == false)
							return (Triangle) ddf.create(
									commande.excute(valeurs.get(0), points.get(0), points.get(1), points.get(2)));

					}
				}
				System.out.println("vous avez inserer deux ou trois point qui ont les memes coordonées");
				return null;
			} else {
				System.out.println("le nom :" + " " + valeurs.get(0) + " existe deja");
				return null;
	...
	private boolean verifieCoordone(Point a, Point b) {
		if (a.getX() == b.getX()) {
			if (a.getY() == a.getY()) {
				return true;
			}
